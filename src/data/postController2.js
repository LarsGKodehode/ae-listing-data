// Defined them in here so I could just test this file, I advise you fold this
// <<<<==== Should be a tiny arrow next to the line numbers
const posts = [
  {
...
class MockApi {
  ...
  request({ uri, verb }) {
    return new Promise((resolve, reject) => {
      const makeRequest = () => {
        const [requestPath, queryString] = uri.split("?");
        const route = this.#routes.find(({ path }) => path === requestPath)

        if (!route) {
          return ({ ok: false, statusCode: 400, message: `URI does not exist: '${uri}'` })
        }

        if (!route[verb]) {
          return ({ ok: false, statusCode: 400, message: `route does not support verb: ${verb}` })
        }

        const data = route[verb](this.#data, queryString)
...
const postRoutes = [
  ...
  },
  {
    path: "/post",
    GET: (db, queryString) => {
      const { author } = parseQuery(queryString)

      if (!author) return false

      const data = db.posts.filter((post) => post.author.username.includes(author))
...
// Setup and configure the Mock API
export const postApi = new MockApi({
  data: {
    posts
  },
  routes: [
...
const tests = [
  { uri: "/posts", verb: "GET" },
  { uri: "/posts/id", verb: "GET" },
  { uri: "/post?author=Einar", verb: "GET" },
  { uri: "/post?author=inar", verb: "GET" },
  { uri: "/post?author=valiantdesignation", verb: "GET" },
]
